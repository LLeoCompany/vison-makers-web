...
export async function refreshAccessToken(
  ...
  try {
    ...
    const isRefreshTokenValid = await verifyPassword(refreshToken, session.refresh_token_hash);
...
// Logout
export async function logoutAdmin(userId: string, sessionId?: string): Promise<ApiResponse<void>> {
  try {
...
// Get user by ID
export async function getUserById(userId: string): Promise<ApiResponse<AdminUserRow>> {
  try {
...
export async function createAdminUser(
  userData: Omit<AdminUserInsert, 'id' | 'password_hash' | 'created_at' | 'updated_at'> & {
    password: string;
  },
  createdBy: string
