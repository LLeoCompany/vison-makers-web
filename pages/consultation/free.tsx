/**
 * VisionMakers 자유 상담 페이지 - Toss UI 스타일
...
const FreeConsultationPage: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    preferredContactTime: state.free.contact.preferredContactTime || 'anytime' as const
  });
...
자세히 설명해주실수록 정확한 상담이 가능해요."
              ...
              {errors.description ? (
                ...
                <p className="mt-2 text-sm text-gray-500">
                  최소 10자 이상 입력해주세요. 자세할수록 정확한 상담이 가능해요.
                </p>
              )}
            </div>
                  ...
                  {contactTimeOptions.map((option) => (
                    <label key={option.value} className="cursor-pointer">
                      <input
                        ...
                        className="sr-only"
                      />
                      <div className={`text-center py-3 px-4 rounded-xl border-2 text-sm font-medium transition-all ${
        ...
        <div className="mt-8 bg-green-50 rounded-2xl p-6 border border-green-100">
          <div className="flex items-start space-x-3">
            ...
            <div>
              <h4 className="font-semibold text-green-900 mb-1">상담 진행 안내</h4>
              <p className="text-sm text-green-800 leading-relaxed">
                신청해주신 내용을 바탕으로 <strong>24시간 이내</strong>에 전문가가 연락드립니다.
                상담은 <strong>완전 무료</strong>이며, 프로젝트에 대한 구체적인 제안서와 견적을 제공해드립니다.
              </p>
            </div>
          </div>
        </div>
