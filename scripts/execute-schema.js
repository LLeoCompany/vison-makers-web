      ...
      try {
        ...
        if (error) {
          ...
          if (altError && altError.message.includes("does not exist")) {
            console.log(
              `  ⚠️  Trying direct execution for: ${statement.substring(
                0,
                50
              )}...`
            );
...
async function executeAllSchemas() {
  console.log("🚀 LeoFitTech Database Schema Setup");
  console.log("=====================================\n");
  ...
  for (const table of tablesToTest) {
    ...
    console.log(
      `  ${exists ? "✅" : "❌"} ${table}: ${exists ? "EXISTS" : "NOT FOUND"}`
    );
  }
  ...
  if (totalErrors === 0) {
    ...
    console.log(
      "🔗 You can now test the connection with: node scripts/test-connection.js"
    );
  } else {
    console.log("\n⚠️  Schema execution completed with some errors.");
    console.log(
      "💡 You may need to run scripts manually in Supabase Dashboard SQL Editor."
    );
  }
}
