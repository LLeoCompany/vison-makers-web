// LeoFitTech Setup Status Checker
...
console.log("🚀 LeoFitTech Setup Status\n");
  ...
  try {
    ...
    if (error) {
      if (error.message.includes("does not exist")) {
        ...
        console.log(
          "📝 Action needed: Execute SQL scripts in Supabase Dashboard"
        );

        console.log("\n🔧 Next Steps:");
        console.log(
          "1. Go to: https://app.supabase.com/project/nwnxbhkaoydiomjnjzdk/sql"
        );
      ...
      console.log(
        `📊 Consultations table: ${data ? data.length : 0} records found`
      );
      return true;
    }
  } catch (error) {
...
async function testFullSetup() {
  ...
  if (schemaReady) {
    ...
    try {
      ...
      if (result.success) {
        console.log(
          `✅ Consultation creation: Working (${result.data.consultationNumber})`
        );
      } else {
